...
elif args.method in 'nodesafe':
    model = NODESafe(d, c, args).to(device)
elif args.method == 'OE':
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        optimizer.zero_grad()
        loss = model.loss_compute(dataset_ind, dataset_ood_tr, criterion, device, args, m_criterion=m_criterion, epoch=epoch)
...
results, best_epoch = logger.print_statistics(return_best_idx=True)
print('best epoch: ' + str(best_epoch.item()))
