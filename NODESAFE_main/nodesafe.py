...
class NODESafe(nn.Module):
    ...
    def detect(self, dataset, node_idx, device, args):
        ...
        # logits = F.normalize(logits, dim=1) * 20
    ...
    def loss_compute(self, dataset_ind, dataset_ood, criterion, device, args, m_criterion = None, sc_criterion = None, epoch=None):
        ...
        if args.use_UB:
            if args.use_reg:
                mloss_in, _ = m_criterion(_features = logits_in[train_in_idx],  labels = dataset_ind.y[train_in_idx].squeeze(1).to(device), _features_out = logits_out, epoch = epoch)
            else:
                mloss_in, _ = m_criterion(_features = logits_in[train_in_idx],  labels = dataset_ind.y[train_in_idx].squeeze(1).to(device), epoch = epoch)
