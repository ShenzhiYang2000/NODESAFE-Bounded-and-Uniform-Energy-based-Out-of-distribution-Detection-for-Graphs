...
class Logger_detect(object):
    ...
    def print_statistics(self, run=None, return_best_idx=False, dataset=None):
        if run is not None:
            result = 100 * torch.tensor(self.results[run])
...
def save_result(results, args):
    ...
    if args.method == 'nodesafe':
        if args.use_prop:
            name = 'nodesafe++' if args.use_reg else 'nodesafe'
        else:
            name = 'nodesafe++ w/o prop' if args.use_reg else 'nodesafe w/o prop'
    else:
        name = args.method
